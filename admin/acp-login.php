...
if(@$_POST['do']=="login")
{
    ...
    $sql      = "SELECT * FROM ".USERS_TABLE." WHERE `username`='".$db->sql_escape($username)."' and `password`='".$db->sql_escape($password)."'";
    if($db->sql_numrows($sql)){
        $result   = $db->sql_query($sql);
        $date     = $db->sql_fetchrow($result);
        $capabilities = get_user_meta($date['id'],'capabilities');
        ...
        {
            $msg = '<div class="alert alert-danger" role="alert">'.get_admin_languages('login_error').'</div>';
        }
