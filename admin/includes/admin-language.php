...
//----------------------------------------------------------------------|
/***********************************************************************|
 * Project:      Creative language Manager                               |
//----------------------------------------------------------------------|
 * @link http://megatpl.com/                                            |
 * @copyright 2014.                                                     |
 * @author Eng Hossam Hamed <megatpl@gmail.com> <themearabia@gmail.com> |
 * @package Creative language Manager                                    |
 * @version 2.0                                                         |
 * Link : http://codecanyon.net/item/creative-language-manager/7888458   |
//----------------------------------------------------------------------|
************************************************************************/
//----------------------------------------------------------------------|
...
class admin_language
{
    ...
    public function index_language()
    {
        ...
        $template->set_filename('language/index_language.html');
        page_footer();
    }
    ...
    public function edit_language()
    {
        ...
        page_header(get_admin_languages('editing').' '.$rowlang['name']);
        $template->set_filename('language/form_language.html');
        page_footer();
    }
            ...
            else
			{
				...
				else
				{
					...
					foreach($phrases as $code => $phrase)
					{
						...
						$sql .= "('".$data['iso_code']."','$code','".$db->sql_escape($phrase)."')";
					}
					$sql .= ";";
    ...
    function get_phrases($lang = NULL,$fields="varname,text")
	{
        ...
        $sql    = "SELECT ".$fields." FROM ".PHRASES_TABLE." WHERE `lang_iso`='".$lang_code."' ORDER BY varname ASC";
        $result = $db->sql_query($sql);
        $data = array();
        while($row = $db->sql_fetchrow($result))
        {
    ...
    function export_lang($id)
	{
		...
		if($lang_details)
		{
			...
			header("Content-Disposition: attachment; filename=\"megatpl_lang_".$lang_details['code'].".xml\""); 
