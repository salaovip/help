...
class admin_article
{
    ...
    public function index_article()
    {
        ...
        $sql          = "SELECT * FROM ".POSTS_TABLE." WHERE posttype='article' ORDER BY modified DESC";
        ...
        {
            $template->assign_block_vars('admin_loop_article', array( 
                'ID'            => $row['id'],
                'TITLE'         => $row['title'], 
                'CATID'         => $row['cid'],
        ...
        $template->assign_vars(array(
            'CLASS_ARTICLE'     => ' class="active"',
            'CLASS_ARTICLEI'    => ' class="active"',
            'SHOW_PAGINATION'   => $showpagination,
            'PAGINATION'        => pagination($total,$limit,$page,'article.php?'),
        ));
        page_header(get_admin_languages('manage_article'));
        $template->set_filename('article.html');
        page_footer();
    }
    

    // form article
    public function form_article($type = 'new')
    {
        ...
        {
            $orders = $db->sql_numrows("SELECT * FROM ".POSTS_TABLE." WHERE posttype='article'") + 1;
            $template->assign_vars(array( 
                'ID'            => 0,
                'STATUS'        => true,
                'ORDERS'        => $orders,
                'CATE'          => $this->get_cat($config['defaultcatarticle']),
                'QUERY'         => 'addnew',
            ));
            $title_page = get_admin_languages('add_new');
            $template->assign_vars(array(
                'CLASS_ARTICLE' => ' class="active"',
                'CLASS_ARTICLEA' => ' class="active"',
            ));
        }
        
        page_header($title_page);
        $template->set_filename('form-article.html');
        page_footer();
    }
    // active article
    ...
    {
        global $db;
        $result = $db->sql_query("SELECT * FROM ".CATEGORIES_TABLE." WHERE catetype='article' ORDER BY id ASC");
