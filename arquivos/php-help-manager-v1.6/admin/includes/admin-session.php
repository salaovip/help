...
class acp_session
{
    ...
    public function get_sessioncp($login = false){
        ...
        $countsession = $db->sql_numrows(
            ...
            `sessionhash`='".@$_SESSION['sessionhashadmincp']."' and `host`='".$ip."' and `useragent`='".$pc."' and `lastactivity` > '".time()."'
        ");
        ...
        if($login == true)
        {
            ...
            if($countsession == true)
            {
                $db->sql_query("
                    ...
                    `sessionhash` = '".$_SESSION['sessionhashadmincp']."' and `host`='".$ip."' and `useragent`='".$pc."'
                ");
            }
            else
            {
    ...
    public function unset_session(){
        ...
        $ip = $_SERVER['REMOTE_ADDR'];
        $pc = $_SERVER['HTTP_USER_AGENT'];
        $db->sql_query("DELETE FROM ".SESSION_TABLE." WHERE `sessionhash`='".$_SESSION['sessionhashadmincp']."' and `host`='".$ip."' and `useragent`='".$pc."'");
        header("Location:acp-login.php"); 
    }
    // get account
    public function get_account($filed){
        ...
        $id      = $row['userid'];
        $result2 = $db->sql_query("SELECT * FROM ".ADMIN_TABLE." WHERE `id`='".$id."'");
    ...
    public function get_permission($per)
    {
        
        global $db;
        $result  = $db->sql_query("SELECT * FROM ".GROUPS_TABLE." WHERE `id`='".$this->get_account('gid')."'");
        $row     = $db->sql_fetchrow($result);
        return $row[$per];
    }
